...
- **展開後のブロック数 50 個**：Slack は `markdown` ブロックを 1 ブロックのままでは扱わず、サーバ側で展開する。**見出し（`#`〜`######`）と区切り線（`---`）はそれぞれ 1 ブロック**になり、その間の連続する本文（段落・リスト・コードブロック・引用）はまとめて 1 ブロックになる。テーブルは 1 個 1 ブロック。この展開後の合計が 50 を超えると `chat.postMessage` が `invalid_blocks` を返す（`response_metadata` は `no more than 50 items allowed [json-pointer:/blocks]`）。**送信するブロック数は 20 個程度でも展開後に 50 を超えることがある**ため、送信側の数を見ていても気づけない。`block_budget.ts` の `checkBlockBudget` で投稿前に見積もり、超過時は内訳を添えて投稿を中止する。段落をいくら増やしても 1 ブロックにまとまるので、効くのは見出し・区切り線・テーブルの数であって本文の長さではない。
